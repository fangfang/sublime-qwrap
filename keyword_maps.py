...
global_reducer = [
	...
	('^el(em)?(?=[^\w]|$)', '__element'),
	('^([a-z](\w*[a-z])?)W(?=[^\w]|$)', '__wrap'),
	('^([A-Z](\w*[a-z])?)W(?=[^\w]|$)', 'W'),	
	('__wrap(\.\w+\(.*?\))+(?=[^\w]|$)', '__wrap'),	#链式调用
	('^[a-z]+El(ement)?(?=[^\w]|$)', '__element'),
	('^e(vt)?(?=[^\w]|$)', '__event'),
	('^[A-Z]([a-z]\w*)(?=[^\w]|$)', '__function'),
	('^function\s*\w*\(.*\).*(?=[^\w]|$)', '__function'),
	('^\/.*\/[gim]{0,3}(?=[^\w]|$)', '__regex'),
	('^(new\s*)?RegExp\(.*?\)(?=[^\w]|$)', '__regex'),
	('^(new\s*)?Object\(.*?\)(?=[^\w]|$)', '__object'),
	('^{.*}(?=[^\w]|$)', '__object'),
	('^(new\s*)?Date\(.*?\)(?=[^\w]|$)', '__date'),
	('^(new\s*)?Array\(.*?\)(?=[^\w]|$)', '__array'),
	('^\[.*\](?=[^\w]|$)', '__array'),
	('^\d+(.\d+)(?=[^\w]|$)', '__number'),
	('"(?:\\.|(\\\\\\")|[^\\""\\n])*"(?=[^\w]|$)', '__string'),
	("'(?:\\.|(\\\\\\')|[^\\''\\n])*'(?=[^\w]|$)", '__string'),
	('^(QW\.)?((DomU?\.)|(W?\.)|(NodeW?\.))?g\(.*?\)(?=[^\w]|$)', '__element'),
	('.*((create)|(get))Element(?!s)\w+\(.*?\)(?=[^\w]|$)', '__element'),
	('^(QW\.)?(W|NodeW)?\(.*?\)(?=[^\w]|$)', '__wrap'),
]